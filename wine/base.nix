    ...
    postInstall =
      ...
      lib.optionalString supportFlags.embedInstallers ''
        ...
        ${lib.strings.concatStringsSep "\n" (
          (map (links "share/wine/gecko") geckos) ++ (map (links "share/wine/mono") monos) ++ (map (links "lib/wine/aarch64-windows") fexes)
        )} 
      ''  # trying to do the same as was done for gecko and mono;
          #if it doesn't work we can copy the fex dll's to $out/lib/wine/aarch64-windows and it should work
