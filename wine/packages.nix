...
with src;
{
  wine32 = pkgsi686Linux.callPackage ./base.nix {
    ...
    fexes = [ (callPackage ../wine-fex/fex.nix) ]; # used in
  ...
  wineWow = callPackage ./base.nix {
    ...
    fexes = [ (callPackage ../wine-fex/fex.nix) ];
  ...
  wineWow64 = callPackage ./base.nix {
    ...
    fexes = [ (callPackage ./fex.nix) ];  # there is only one fex package but
                                          # using lists to keep same format
                                          # as mono and gecko

                                          # fexes used in base.nix
    
    configureFlags = [
      # "--enable-archs=x86_64,i386"
      "--enable-archs=arm64ec,aarch64,i386" # changed to allow arm64ec
    ];
    platforms = [
      # "x86_64-linux"
      # "x86_64-darwin"
      "aarch64-linux" # obviously this only works on aarch now
    ];
    mainProgram = "wine";
  };
}
